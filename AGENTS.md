...
- **Installable app:** `public/manifest.webmanifest`, the icons in `public/icons/` (change the SVGs, then `npm run icons`), `public/sw.js` and `public/offline.html`. The service worker registers only in production builds and must never cache API responses or people's data; `src/pwa.test.ts` checks it. Bump its `CACHE` name when you change the offline files.
